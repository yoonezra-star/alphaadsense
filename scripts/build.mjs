...
<div class="tool-result-grid" id="score-detail" aria-live="polite"></div>
<h2>점수 해석</h2>
...
<h2>진단 결과를 기록하는 방법</h2>
<p>점수를 한 번만 보고 끝내기보다 신청 전 3일, 신청 전 1일, 신청 직전에 같은 항목을 다시 체크해 보세요. 점수가 오른 영역과 아직 낮은 영역을 남기면 재신청이 필요할 때도 수정 방향을 빠르게 찾을 수 있습니다.</p>
<script>
(function () {
  ...
  const groups = Array.from(form.querySelectorAll("fieldset")).map((fieldset) => ({
    title: fieldset.querySelector("legend").textContent,
    inputs: Array.from(fieldset.querySelectorAll("input"))
  }));
  const guideLinks = {
    "필수 페이지와 신뢰": [["소개 페이지", "/about/"], ["개인정보처리방침", "/privacy/"], ["운영 원칙", "/editorial-policy/"]],
    "콘텐츠와 탐색": [["승인 친화 글 구성", "/guides/approval-friendly-article-format/"], ["사이트 구조 점검", "/guides/site-structure-audit-example/"], ["내부 링크 가이드", "/guides/internal-linking-guide/"]],
    "기술과 색인": [["색인 확인 방법", "/guides/indexing-before-adsense/"], ["Cloudflare 예시", "/guides/cloudflare-search-console-example/"], ["접근 불가 해결", "/guides/site-down-unavailable/"]],
    "모바일과 정책": [["금지 콘텐츠 점검", "/guides/prohibited-content-check/"], ["쿠키 고지 기본", "/guides/cookie-notice-basics/"], ["면책 고지 작성", "/guides/disclaimer-page-guide/"]]
  };
  function render() {
    ...
    let status = "보강 필요";
    let summary = "필수 페이지, 대표 글, 기술 접근성 중 비어 있는 영역을 먼저 채우는 단계입니다.";
    if (score >= 85) message = "신청 전 최종 점검 단계로 볼 수 있습니다.";
    if (score >= 85) {
      status = "신청 직전 점검";
      summary = "기본 신뢰 신호와 탐색 흐름은 갖춰졌습니다. 색인, 모바일, 정책 문구만 마지막으로 확인하세요.";
    } else if (score >= 70) {
      message = "큰 틀은 갖췄지만 약한 영역을 더 보강하세요.";
      status = "부분 보강";
      summary = "신청은 검토할 수 있지만 낮은 영역이 있으면 거절 사유가 될 수 있습니다. 부족한 그룹부터 보강하세요.";
    }
    output.innerHTML = "<strong>" + score + "점</strong><span>" + message + "</span>";
    const rows = groups.map((group) => {
      const max = group.inputs.reduce((sum, input) => sum + Number(input.dataset.points), 0);
      const earned = group.inputs.filter((input) => input.checked).reduce((sum, input) => sum + Number(input.dataset.points), 0);
      const missing = group.inputs.filter((input) => !input.checked).map((input) => input.parentElement.textContent.trim());
      const links = guideLinks[group.title].map((link) => "<a href=\\"" + link[1] + "\\">" + link[0] + "</a>").join("");
      return "<article class=\\"result-card\\"><p class=\\"eyebrow\\">" + group.title + "</p><h3>" + earned + " / " + max + "점</h3><p>" + (missing[0] || "이 영역은 기본 점검을 통과했습니다.") + "</p><div class=\\"mini-links\\">" + links + "</div></article>";
    }).join("");
    detail.innerHTML = "<section class=\\"result-summary\\"><strong>" + status + "</strong><p>" + summary + "</p></section>" + rows;
  }
...
(function () {
  const data = {
    low: { title: "가치가 낮은 콘텐츠", priority: "대표 글 깊이 보강", evidence: "수정 전후 글 목록, 추가한 표와 예시, 병합한 중복 글을 기록합니다.", points: ["대표 글의 깊이와 중복성을 먼저 봅니다.", "문제 상황, 판단 기준, 수정 예시, 체크리스트를 추가합니다.", "검색 결과 요약처럼 보이는 일반론을 줄입니다."], links: [["낮은 가치 수정 예시", "/guides/low-value-rejection-rewrite-example/"], ["승인 친화적 글 구성", "/guides/approval-friendly-article-format/"]] },
    nav: { title: "탐색 문제", priority: "메뉴와 허브 재정리", evidence: "상단 메뉴, 하단 링크, 카테고리 허브, 깨진 링크 검사 결과를 기록합니다.", points: ["상단 메뉴와 하단 링크가 같은 기준으로 연결되는지 봅니다.", "빈 카테고리와 깨진 링크를 제거합니다.", "카테고리 페이지에 읽는 순서를 추가합니다."], links: [["사이트 구조 점검", "/guides/site-structure-audit-example/"], ["탐색 문제 해결", "/guides/navigation-issue-fix/"]] },
    access: { title: "접근 불가", priority: "도메인과 HTTPS 안정화", evidence: "루트, www, sitemap, robots, Cloudflare Pages 상태와 확인 날짜를 기록합니다.", points: ["루트 도메인과 www 주소가 모두 열리는지 확인합니다.", "HTTPS 인증서와 Cloudflare Pages 커스텀 도메인 상태를 봅니다.", "봇 접근을 막는 보안 규칙을 최소화합니다."], links: [["사이트 접근 문제 해결", "/guides/site-down-unavailable/"], ["Cloudflare 등록 예시", "/guides/cloudflare-search-console-example/"]] },
    policy: { title: "정책 위험", priority: "위험 문구와 필수 고지 정리", evidence: "삭제한 표현, 보강한 정책 페이지, 공식 자료 확인일을 기록합니다.", points: ["금지 콘텐츠, 클릭 유도, 승인 보장 표현을 제거합니다.", "개인정보처리방침과 쿠키 정책의 광고 관련 문구를 확인합니다.", "수익 보장과 과장된 표현을 면책 고지 기준으로 낮춥니다."], links: [["금지 콘텐츠 점검", "/guides/prohibited-content-check/"], ["정책 페이지 예시", "/guides/privacy-cookie-page-example/"]] },
    unknown: { title: "복합 문제", priority: "전체 흐름 재점검", evidence: "홈, 대표 글, 정책 페이지, 색인 상태를 같은 표로 묶어 기록합니다.", points: ["홈, 카테고리, 대표 글, 정책 페이지를 같은 흐름으로 점검합니다.", "Search Console에서 발견된 주요 URL과 실제 공개 URL을 비교합니다.", "수정 기록을 남기고 일주일 점검표로 안정화합니다."], links: [["승인 준비 허브", "/approval-hub/"], ["7일 점검 기록", "/guides/seven-day-pre-apply-log-example/"]] }
  };
  ...
  function render() {
    ...
    result.innerHTML = "<section class=\\"result-summary\\"><strong>" + item.title + "</strong><p>우선순위: " + item.priority + "</p></section><div class=\\"tool-result-grid compact\\"><article class=\\"result-card\\"><p class=\\"eyebrow\\">먼저 볼 항목</p><ul>" + item.points.map((point) => "<li>" + point + "</li>").join("") + "</ul></article><article class=\\"result-card\\"><p class=\\"eyebrow\\">수정 증거</p><p>" + item.evidence + "</p><div class=\\"mini-links\\">" + item.links.map((link) => "<a href=\\"" + link[1] + "\\">" + link[0] + "</a>").join("") + "</div></article></div>";
  }
...
(function () {
  const plans = {
    new: { title: "기초 구축형 30일 계획", goal: "신뢰 페이지와 대표 카테고리를 먼저 만들고, 신청 가능한 기본 구조를 갖춥니다.", weeks: ["1주차: 소개, 문의, 개인정보, 쿠키, 면책 고지, 시작 가이드를 완성합니다.", "2주차: 승인 준비, 글쓰기, 정책 체크 카테고리의 대표 글을 작성합니다.", "3주차: 거절 해결 사례와 체크리스트형 글을 추가합니다.", "4주차: 내부 링크, sitemap, 모바일 화면, 검색 페이지를 점검합니다."], links: [["시작 가이드", "/start-guide/"], ["정책 체크", "/policy/"], ["신청 전 체크리스트", "/checklist/"]] },
    content: { title: "콘텐츠 보강형 30일 계획", goal: "새 글보다 기존 대표 글의 깊이, 표, 예시, 내부 링크를 보강합니다.", weeks: ["1주차: 짧고 반복되는 글을 병합하고 대표 글을 정합니다.", "2주차: 각 대표 글에 표, 예시, 수정 기록, 관련 글 링크를 추가합니다.", "3주차: 정책 페이지와 공식 자료 링크를 보강합니다.", "4주차: Search Console, 내부 링크, 카테고리 허브를 재점검합니다."], links: [["글쓰기 가이드", "/writing/"], ["낮은 가치 수정", "/guides/low-value-content-fix/"], ["내부 링크 가이드", "/guides/internal-linking-guide/"]] },
    reject: { title: "재신청 안정화형 30일 계획", goal: "거절 원인별 수정 기록을 남기고, 바로 재신청하기보다 안정화 기간을 확보합니다.", weeks: ["1주차: 거절 메시지를 분류하고 수정 대상 글 목록을 만듭니다.", "2주차: 낮은 가치 또는 탐색 문제에 해당하는 글과 메뉴를 먼저 고칩니다.", "3주차: 모바일, HTTPS, sitemap, 정책 문구를 다시 확인합니다.", "4주차: 7일 점검 기록을 남기고 안정화 후 재신청을 검토합니다."], links: [["거절 사유 진단", "/tools/rejection-diagnosis/"], ["7일 점검 기록", "/guides/seven-day-pre-apply-log-example/"], ["승인 점수 진단", "/tools/approval-score/"]] }
  };
  ...
  function render() {
    ...
    output.innerHTML = "<section class=\\"result-summary\\"><strong>" + plan.title + "</strong><p>" + plan.goal + "</p></section><div class=\\"tool-result-grid\\">" + plan.weeks.map((item, index) => "<article class=\\"result-card\\"><p class=\\"eyebrow\\">Week " + (index + 1) + "</p><h3>" + item.split(':')[0] + "</h3><p>" + item.replace(/^.*?:\\s*/, '') + "</p></article>").join("") + "</div><div class=\\"mini-links planner-links\\">" + plan.links.map((link) => "<a href=\\"" + link[1] + "\\">" + link[0] + "</a>").join("") + "</div>";
  }
  select.addEventListener("change", render);
  render();
}());
...
.tool-result-grid { display: grid; grid-template-columns: repeat(2, 1fr); gap: 14px; margin: 18px 0 28px; }
.tool-result-grid.compact { margin-bottom: 6px; }
.result-summary { border: 1px solid var(--line); border-radius: 8px; padding: 16px 18px; background: #eef6ff; margin-bottom: 14px; }
.result-summary strong { display: block; color: #1e3a8a; font-size: 18px; margin-bottom: 4px; }
.result-summary p { color: var(--ink); margin: 0; }
.result-card { border: 1px solid var(--line); border-radius: 8px; padding: 18px; background: white; box-shadow: 0 6px 18px rgba(21,32,51,.04); }
